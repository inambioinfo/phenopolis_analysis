...
first variant, if its cadd is lower than min_cadd_for_second_variant.
min cadd for second variant is there is because variants
with cadd higher than it is automatically assumed to be bad.
This essentially adds more weight to high cadd variants
'''
def get_variants(**kwargs):
    ...
    if kwargs['mode'] == 'r':
        broad_vs = (k for k,v in kwargs['variants'].items() 
                ...
                and v['cadd'] >= min(
                    kwargs['cr'][0],
                    kwargs['min_cadd_for_second_variant']
                    )
                )
...
def phenogenon(**kwargs):
    ...
    raw_p_h = set([kk for kk,vv in kwargs['patient_info'].items() if set(hpos).issubset(set(vv))])
    # make a np matrix
    shape1,shape2 = [],[]
    ...
    for k,v in kwargs['patient_map'].items():
        ...
        p_gh = 0
        for p in v[0]:
            if set(hpos).issubset(set(kwargs['patient_info'][p])):
                p_gh += 1
        pval = fisher.pvalue(
                p_a - p_h - p_g + p_gh, 
                p_h - p_gh, 
